...
\input{figures/F11_common_shock_geometry.tex}

\Cref{fig:common-shock-geometry} quantifies the size of the
misspecification under \cref{ex:common-shock}. Under positive same-sign
loadings the misspecified observed-axes constant under-states the true
tail by roughly a factor of $2.8$ across the threshold range, while the
latent-shock constant tracks the empirical tail within sampling error.
This number is what \cref{thm:latent-shock-tail} predicts: the
ratio of the two constants is $\sum_i b_i^\alpha / (\sum_i b_i)^\alpha$
which equals $\sim 0.36$ for the four-loading design used in the
figure, so the misspecified estimator under-states by a factor of
$1/0.36 \approx 2.8$.
...
\begin{figure}[t]
\centering
\input{diagrams/D9_estimator_decision_tree.tex}
