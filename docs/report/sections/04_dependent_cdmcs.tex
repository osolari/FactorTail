...
\caption[Common-shock geometry (planned)]{\textbf{(PLANNED).} Common-shock
geometry. The final figure will show simulated observations from
\(X_i=b_iZ_0+E_i\), highlighting that joint extremes align with the
common-shock ray. The independent estimator assigns mass to coordinate axes,
while the latent-shock estimator assigns mass to the ray. Source schema:
\texttt{results/F11\_common\_shock\_geometry.csv}.}
